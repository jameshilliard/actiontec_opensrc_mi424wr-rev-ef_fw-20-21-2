...
static inline const char *get_version(void)
{
    ...
    const char *cvs_tag = "$Name: tag-aei-bhr-4-7-XX-21-2 $";
...
#define FLASH_SIZE_5M 0x500000
        {
         if (size > FLASH_SIZE_16M)
         {
          // mtdblock0 : __ptr[0]
          // mtdblock1 : __ptr[1]
          // mtdblock2 : __ptr[2]
          static struct mtd_partition __prt[3] = {
           {.name = "openrg", .offset = 0, .size = FLASH_SIZE_16M},
           {.name = "osgifs", .offset = 0, .size = FLASH_SIZE_5M},
           {.name = "osgirw", .offset = 0, .size = 0}
          };
          __prt[1].offset = __prt[0].offset + __prt[0].size;
          __prt[2].size = size - __prt[0].size - __prt[1].size;
          __prt[2].offset = __prt[1].offset + __prt[1].size;
          physmap_set_partitions(__prt, 3);
         }
        }
#endif
